...
describe CrystalNotoCJK do
  describe "VARIANTS" do
    it "lists the four official Noto Sans CJK regional variants" do
      CrystalNotoCJK::VARIANTS.should eq([:sc, :tc, :jp, :kr])
    end
  end
  ...
  describe ".font_path" do
    it "returns nil when the cache is empty" do
      ...
      pending "skipped if local cache is populated" if CrystalNotoCJK.populated?
      CrystalNotoCJK.font_path.should be_nil
    end

    it "returns nil for an unknown variant identifier" do
      # `:xx` isn't a real variant — should not raise, just return nil.
      CrystalNotoCJK.font_path(:xx).should be_nil
    end
  end

  describe ".populated?" do
    it "is a boolean" do
      [true, false].should contain(CrystalNotoCJK.populated?)
    end
  end
end

describe CrystalNotoCJK::Cache do
  describe "VARIANT_FILES" do
    it "covers every variant" do
      CrystalNotoCJK::Cache::VARIANT_FILES.keys.to_a.sort.should eq([:jp, :kr, :sc, :tc])
    end

    it "uses the upstream Noto file naming convention" do
      CrystalNotoCJK::Cache::VARIANT_FILES[:sc][:file].should eq("NotoSansCJKsc-Regular.otf")
      CrystalNotoCJK::Cache::VARIANT_FILES[:jp][:file].should eq("NotoSansCJKjp-Regular.otf")
    end
  end
  ...
  describe ".dir" do
    it "honours CRYSTAL_NOTO_CJK_CACHE_DIR if set" do
      ...
      begin
        CrystalNotoCJK::Cache.dir.should eq("/tmp/test-noto-cjk-cache")
      ensure
        ENV.delete("CRYSTAL_NOTO_CJK_CACHE_DIR")
      end
    end

    it "uses /var/cache/crystal-noto-cjk for system mode" do
      CrystalNotoCJK::Cache.dir(system: true).should eq("/var/cache/crystal-noto-cjk")
    end
    ...
    it "falls back to a user-owned cache path otherwise" do
      ...
      path = CrystalNotoCJK::Cache.dir
      path.should contain("crystal-noto-cjk")
  ...
  describe ".path" do
    it "returns nil for an unknown variant" do
      CrystalNotoCJK::Cache.path(:xx).should be_nil
    end
    ...
    it "returns nil when the variant file is not on disk" do
      ...
      begin
        CrystalNotoCJK::Cache.path(:sc).should be_nil
      ensure
        ENV.delete("CRYSTAL_NOTO_CJK_CACHE_DIR")
      end
    end
  ...
  describe ".installed" do
    it "returns an empty array on a fresh cache" do
      ENV["CRYSTAL_NOTO_CJK_CACHE_DIR"] = "/tmp/empty-noto-cache-#{Random.rand(1_000_000)}"
      begin
        CrystalNotoCJK::Cache.installed.should be_empty
      ensure
        ENV.delete("CRYSTAL_NOTO_CJK_CACHE_DIR")
      end
    end
